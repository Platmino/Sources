...
local deb = function(t)
	if _G.DEBUG_MODE then 
		print("[Obese.vip]: "..tostring(t))
	end
end
...
local scriptloadstring = [[task.wait(5) loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/2f16ad78baf38b2da6204effe19036b3.lua"))()]]

local Tick = tick()
local OldFont = "https://github.com/f1nobe7650/other/raw/main/ProggyTiny.ttf"
local MCFont = "https://raw.githubusercontent.com/laagginq/Alysum/main/backend/fonts/arial.ttf"
local ObeseFont = "https://raw.githubusercontent.com/laagginq/obese.vip/main/QuartzoBold-W9lv.ttf"
local OBImg = "https://raw.githubusercontent.com/laagginq/obese.vip/main/Icon.png"

local OBESEVERSION = "v0.0.1 Beta"
local MainDir = "obese.vip"
local AssetsDir = MainDir.."\\assets"
local ConfigsDir = MainDir.."\\configs"
local obesecache = MainDir.."\\cache"
...
if not isfolder(obesecache) then 
    makefolder(obesecache)
end
-- Custom Fonts
do 
	--
	writefile(AssetsDir.."/obese_font.ttf", game:HttpGet(OldFont))
	-- 
		...
		faces = {
			{
				...
				weight = 400,
				style = "normal",
				assetId = getcustomasset(AssetsDir.."/obese_font.ttf")
			}
		}
	}
...
local function getCustomImage(imageUrl,name)
	local filename = AssetsDir.."\\"..tostring(name)..".png"
	if not isfile(filename) then
		local response = http_request({Url = imageUrl})
...
local function getGameIcon(gameId)
	local filename = AssetsDir.."\\"..tostring(gameId)..".png"
	if not isfile(filename) then
		local URL = "https://thumbnails.roblox.com/v1/places/gameicons?placeIds="..gameId.."&returnPolicy=PlaceHolder&size=50x50&format=Png&isCircular=false"
		local HttpService = game:GetService("HttpService")
		local response = http_request({Url = URL})
